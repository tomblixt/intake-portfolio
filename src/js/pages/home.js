  ...
  animateIn() {
    return new Promise((resolve) => {
      ...
      timeline
        ...
        .to(this.revealText, { y: 0, ease: Power2.easeOut }, delay + 0.2)
        .to(
          this.links,
          { y: 0, alpha: 1, width: 160, ease: Power2.easeOut, stagger: 0.3 },
          delay + 0.8
        );
    });
  }
  listeners() {
    this.links.forEach((link, index) => {
      link.addEventListener(
        "mouseenter",
        this.enterLink.bind(this, index, link)
      );
  ...
  enterLink(i, item) {
    ...
    this.links.forEach((link, index) => {
      if (i === 0 && index !== 0) {
        ...
        gsap.to(link, 0.4, { alpha: index < 3 ? 0.9 : 0.8 });
      } else if (i === 2 && index !== 2) {
        gsap.to(link, 0.4, { alpha: index > 0 ? 0.9 : 0.8 });
      } else if (i === 3 && index !== 3) {
